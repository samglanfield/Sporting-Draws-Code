...
system.time(for (r in 1:nsim) {
  ...
  while (length(which(is.na(initialgroups[1, 1, ]) == TRUE)) > 0) {
    #this section only selects valid teams from the possible valid complete draws remaining
...
countMatrix = countMatrix / length(drawsS)
countMatrix
...
for (i in 1:100) {
  low = (1000 * (i - 1) + 1)
  up = 1000 * i
  assign(paste("subdrawsCL", i, sep = ""), drawsS[index[low:up]], envir =
           .GlobalEnv)
...
x = which(countMatrix %in% min(countMatrix[which(countMatrix > 0)]))
...
for (i in 1:length(drawsS)) {
  ...
  countMatrixu = countMatrixu / i
  countMatrix = countMatrix / i
  
  indexS = which(countMatrix == 0)
  indexU = which(countMatrixu == 0)
  if (length(indexU) >= length(indexS)) {
    index = indexU
  } else {
    index = indexS
  }
  HeVector[i] = CalcHellingerDist(c(countMatrix[-index]), c(countMatrixu[-index]))
