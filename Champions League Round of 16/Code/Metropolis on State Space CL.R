...
for (i in 1:1000) {
  ...
  if (u >= exp(-beta.t * (TVOld - TVNew))) {
    #Rejected, acceptance already done if no changes made
...
results = list(initialstate, TVvector, finalstate)

save(results, file = "Metropolis on State Space CL Draw 10.RData")
