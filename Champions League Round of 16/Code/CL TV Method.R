...
powerset <- function(x) {
  sets <- lapply(1:(length(x)), function(i)
    combn(x, i, simplify = F))
...
countU = matrix(0, nrow = 255, ncol = 255)
countS = matrix(0, nrow = 255, ncol = 255)
...
partialresults = list(countU, countS)
#save(partialresults,file="TV CL Method Draw 1.RData")
